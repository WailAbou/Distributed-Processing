...
class Message:
    ...
    def get_args(self, message_type):
        if message_type == MessageTypes.PROPOSE: return f'v={self.destination.value}'
            ...
            prior = f"n={self.source.process_id}, v={self.source.value}" if self.source.value else None
            return f'n={self.destination.process_id} (Prior: {prior})'
        elif message_type == MessageTypes.ACCEPT: return f'n={self.source.process_id} v={self.source.value}'
        elif message_type == MessageTypes.ACCEPTED: return f'n={self.destination.process_id} v={self.destination.value}'
